...
public class BusConfig {
    ...
    @Bean
    public Replicator replicator(ObjectMapper mapper, Writer writer, InstanceDao instanceDao, EntityDao entityDao) {
        return new ReplicationService(mapper, writer, instanceDao, entityDao);
    }

    @Bean
    public RestReceiver restReceiver(RestTemplate restTemplate, BusHandler busHandler) {
        return new BusRestReceiver(integrationUrl, restTemplate, busHandler);
    }

    @Bean
    public BusHandler busHandler(ObjectMapper mapper, Writer writer, InstanceDao instanceDao, EntityDao entityDao) {
        return new MessageHandler(mapper, writer, instanceDao, entityDao, deanAddress);
    }

    @Bean
    public Writer writer(ObjectMapper mapper, MessageSender messageSender, InstanceDao instanceDao) {
    ...
    @Bean
    public InstanceDao instanceDao(EntityNameConverter entityNameConverter, JdbcTemplate jdbcTemplate) {
